      ...
      // 조회된 레코드가 있으면 성공 응답 전송
        ...
        // 조회 결과에서 사용자 이름 확인
				...
				req.app.render('login_success', context, function(err,html){
					if(err){
						console.error('(auth user) View Rendering 중 Error 발생 : ' + err.stack);
						res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
						res.write('<h2>View Rendering 중 오류 발생</h2>');
						res.write('<p>' + err.statck + '</p>');
						res.end();
        ...
        return;
			...
			if (addedUser) {
				...
				req.app.render('adduser', context, function(err,html){
					if(err){
						console.error('(add user) View Rendering 중 Error 발생 : ' + err.stack);
						res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
						res.write('<h2>View Rendering 중 오류 발생</h2>');
						res.write('<p>' + err.statck + '</p>');
						res.end();
        ...
        return;
			...
			if (results) {
				...
				req.app.render('listuser', context, function(err,html){
					if(err){
						console.error('(list user)View Rendering 중 Error 발생 : ' + err.stack);
						res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
						res.write('<h2>View Rendering 중 오류 발생</h2>');
						res.write('<p>' + err.statck + '</p>');
						res.end();
